...
export default function ElliPricingCard({ provider, theme }: Props) {
...
function Row({ label, value, highlight, theme }: { label: string; value: string; highlight?: boolean; theme: Theme }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
      ...
      <span style={{ fontSize: 12, fontWeight: 600, color: highlight ? '#00C896' : theme.text, whiteSpace: 'nowrap' }}>{value}</span>
    </div>
  );
}
